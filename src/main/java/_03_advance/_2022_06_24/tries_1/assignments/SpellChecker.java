package _03_advance._2022_06_24.tries_1.assignments;

import java.util.HashMap;

public class SpellChecker {
    public int[] solve(String[] A, String[] B) {
        int[] res = new int[B.length];
        Node root = new Node();
        for (int i = 0; i < A.length; i++) {
            String word = A[i];
            add(word, root);
        }
        for (int i = 0; i < B.length; i++)
            res[i] = find(B[i], root) ? 1 : 0;

        return res;
    }

    public void add(String word, Node r) {
        int n = word.length();
        for (int i = 0; i < n; i++) {
            char ch = word.charAt(i);
            if (!r.hm.containsKey(ch)) {
                Node temp = new Node();
                r.hm.put(ch, temp);
                r = temp;
            } else
                r = r.hm.get(ch);
        }
        r.isEnd = true;
    }

    public boolean find(String word, Node r) {
        int n = word.length();
        for (int i = 0; i < n; i++) {
            char ch = word.charAt(i);
            if (!r.hm.containsKey(ch))
                return false;
            else
                r = r.hm.get(ch);
        }
        return r.isEnd;
    }

    class Node {
        boolean isEnd;
        HashMap<Character, Node> hm;

        Node() {
            isEnd = false;
            hm = new HashMap<>();
        }
    }

}

/*
Given an array of words A (dictionary) and another array B (which contain some words).
You have to return the binary array (of length |B|) as the answer where 1 denotes that the word is
present in the dictionary and 0 denotes it is not present. Formally, for each word in B, you need
to return 1 if it is present in Dictionary and 0 if not.
Such problems can be seen in real life when we work on any online editor (like Google Documnet),
if the word is not valid it is underlined by a red line.
NOTE: Try to do this in O(n) time complexity.

Problem Constraints
1 <= |A| <= 1000
1 <= sum of all strings in A <= 50000
1 <= |B| <= 1000

Input Format :
First argument is array of strings A.
First argument is array of strings B.

Output Format : Return the binary array of integers according to the given format.

Input 1:  A = [ "hat", "cat", "rat" ] B = [ "cat", "ball" ] Output 1: [1, 0]
Explanation 1: Only "cat" is present in the dictionary.

Input 2: A = [ "tape", "bcci" ] B = [ "table", "cci" ] Output 2: [0, 0]
Explanation 2: None of the words are present in the dictionary.
 */