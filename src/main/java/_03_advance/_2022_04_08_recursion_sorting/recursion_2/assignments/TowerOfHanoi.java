package _03_advance._2022_04_08_recursion_sorting.recursion_2.assignments;
...
public class TowerOfHanoi {
    public ArrayList<ArrayList<Integer>> towerOfHanoi(int A) {
        ...
        TOH(A, 1, 2, 3, response);
        return response;
    }

    private void TOH(int A, int start, int temp, int stop, ArrayList<ArrayList<Integer>> response) {
        if (A == 0) return;
        TOH(A - 1, start, stop, temp, response);
        ArrayList<Integer> out = new ArrayList<>();
        out.add(A);
        out.add(start);
        out.add(stop);
        response.add(out);
        TOH(A - 1, temp, start, stop, response);
    }
...
You have to return a 2D array of dimensions M x 3, where M is the minimum number of moves
...
Explanation 2:
We can see that this the only unique path with minimal moves to move all disks from
the first to the third tower.
 */