package _01_beginner._2021_12_15.assignments;
...
public class FirstMissingPositiveInteger {
    ...
    // alternate approach without using constant space
    public static int firstMissingPositive(List<Integer> A) {
        // 1. answer would always be in the range 1 to n, (n+1 in special case)
        // 2. remove unwanted numbers & negative numbers : i.e., remove arr[i] if arr[i] > n
        // 3. put the rest of numbers in its correct position (correct position = index number of array)
        for (int i = 0; i < A.size(); i++) {
            System.out.println("i = " + i + " " + A);
            if (A.get(i) < A.size() && A.get(i) >= 0){
                int pos = A.get(i) - 1; // -1 because of zero based indexing
                if (A.get(pos) != A.get(i)){ // swap
                    int temp = A.get(pos);
                    A.set(pos, A.get(i));
                    A.set(i, temp);
                    i--;
        ...
        for (int i = 0; i < A.size(); i++) {
            if (A.get(i) != i+1) return i+1; // +1 because of zero based indexing
        }
        // if all numbers are contigious and already in place
        return A.size()+1;
    }
    public static int firstMissingPositive2(List<Integer> A) {
